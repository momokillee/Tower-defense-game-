/* game.js - externalized JS for the Tower Defense page
   Features:
   - robust sprite-sheet detection & previews
   - drag & drop tower placement from palette
   - floating tower menu (Upgrade / Delete)
   - global targeting toggle (First / Strongest / Weakest)
...
// UI elements (wired in initializeGame)
...
// click handler: open menu on tower click, otherwise try to build
...
function draw() {
    ...
    renderQueue.forEach(item => {
        ...
        if (item.type) {
            ...
            if (asset) ctx.drawImage(asset, item.x, item.y - asset.height);
        }
    });
...
async function initializeGame() {
    ...
    try {
        ...
        canvas.addEventListener('drop', (ev) => {
            ...
            const tile = map.grid[r][c];
            if (tile.type !== 'grass' || tile.tower) return;

            const cost = TOWER_COSTS[0];
            if (gameState.gold < cost) { alert('Not enough gold to build tower.'); return; }
        ...
        if (menuUpgradeBtn) {
            menuUpgradeBtn.addEventListener('click', (e) => {
                ...
                else if (towerMenuTitle) towerMenuTitle.textContent = `Tower (Lvl ${selectedTower.level})`;
            });
        }
        if (menuDeleteBtn) {
...
window._TD = { loadedAssets, spriteMeta, gameState, spawnWave, resetGame: () => {
    if (activeWaveTimeout) clearTimeout(activeWaveTimeout);
    gameState.gold = 100; gameState.lives = 20; gameState.wave = 0; gameState.score = 0;
    gameState.enemies = []; gameState.towers = []; gameState.projectiles = [];
    waveInProgress = false; selectedTower = null;
    if (map) {
        for (let r=0;r<map.rows;r++) for (let c=0;c<map.cols;c++) map.grid[r][c].tower = null;
    }
